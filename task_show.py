...
def move_camera(dt):
    ...
    if keys[key.A]:
        ...
        print 'Command: Let the cow move;'
    if keys[key.D]:
        world.scene.camera.rotation.y -= 13 * dt;#world.scene.camera.rotation.z -= 13 * dt;    
    if keys[key.W]:
        world.objs[4].move = "MOVE";    
        print 'Command: Let the temperature of cow be high;'
    if keys[key.Q]:
        world.objs[2].temp = "TEMP_HIGH";
        print 'Command: Let the temperature of desk be normal;'
    if keys[key.E]:
        world.objs[4].temp = "TEMP_NORMAL";            
        print 'Command: Tasks Finished;'
